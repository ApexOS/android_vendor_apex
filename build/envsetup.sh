function __print_apex_functions_help() {
cat <<EOF
...
- apexgerrit:   A Git wrapper that fetches/pushes patch from/to LineageOS Gerrit Review.
- apexrebase:   Rebase a Gerrit change and push it again.
- apexremote:   Add git remote for LineageOS Gerrit Review.
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/apex-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.apex.device | grep -q "$APEX_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $APEX_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function apexremote()
{
    ...
    local APEX_USER=$(git config --get review.review.apexos.org.username)
    if [ -z "$APEX_USER" ]
    then
        git remote add apex ssh://review.apexos.org:29418/$PFX$PROJECT
    else
        git remote add apex ssh://$APEX_USER@review.apexos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'apex' created"
}
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.apex.device | grep -q "$APEX_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        apexremote
        git push apex HEAD:refs/heads/'$1'
    fi
    '
}
...
function apexgerrit() {
    ...
    local user=`git config --get review.review.apexos.org.username`
...
function apexrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        ...
        echo "      apexrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.apexos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.apex.device | grep -q "$APEX_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/apex/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $APEX_FIXUP_COMMON_OUT ]; then
