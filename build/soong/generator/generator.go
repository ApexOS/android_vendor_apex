...
// Copyright (C) 2018,2021 The ApexOS Project
//
...
func init() {
	android.RegisterModuleType("apex_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = apexExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = apexExpandVariables(ctx, dep_file)
	...
	cmd := apexExpandVariables(ctx, String(g.properties.Cmd))
